...
function Teacher() {
  ...
  const getData = async () => {
    let responce = await fetch(`${api}/viewTeacher/${id}`, {
      method: "GET",
      headers: {},
    });
    let data = await responce.json();
    setTeacherData(data.res);
  };
  useEffect(() => {
    getData();
  }, []);
          ...
          <div className="flex flex-wrap items-center space-y-9 md:space-y-0">
            <div className="xl:w-1/2 order-2 md:order-1 mt-5 sm:mt-0 w-full md:w-[65%] space-y-6 xl:pe-9">
              <div className="text-[22px] lg:text-[30px] xl:text-[40px]">
                <div className="flex  justify-between items-center">
                  <h3 className="text-2xl capitalize">{TeacherData.name}</h3>
                  ...
                  <div>
                    {TeacherData.address && (
                      <div className="text-sm">
                        {TeacherData.address} ,{" "}
                        <span className="capitalize">{Teacher.city} </span>,{" "}
                        <span className="capitalize">{Teacher.state}</span>
                      </div>
                    )}
                  </div>
                </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-4">
                  {TeacherData.area == "education" ? (
                    <>
                      {
                        <div className="flex gap-2">
                          <GoChecklist className="lg:text-xl mt-1 text-blue-600" />
                          <div className="sm:text-xl">
                            {TeacherData.classFrom}th - {Teacher.classTo}th
                            <h6 className="text-sm mt-[-4px]">class</h6>
                          </div>
                        </div>
                      }
                      {TeacherData.science.map((item, index) => (
                        <div
                          key={index}
                      ...
                      {TeacherData.arts.map((item, index) => (
                        <div
                          key={index}
                          className="flex uppercase sm:text-xl gap-2"
                        >
                      ...
                      {TeacherData.commerce.map((item, index) => (
                        <div
                          key={index}
                          className="flex uppercase sm:text-xl gap-2"
                        >
                      ...
                      {TeacherData.exams.map((item, index) => (
                        <div
                          key={index}
                          className="flex uppercase sm:text-xl gap-2"
                        >
              ...
              <div className="flex gap-8 flex-wrap">
                <div>
                  ...
                  <div className="">
                    ${TeacherData.fee}/
                    {(Teacher.feeAs = "perhour" ? "hr" : "month")}
                  </div>
                </div>
                <div>
                  <h4 className="text-2xl my-3 ">Location</h4>
                  <div className="capitalize">{TeacherData.type}</div>
                </div>
                <div>
                  <h4 className="text-2xl my-3 ">Language</h4>
                  <div className="capitalize">
                    {TeacherData.language == "regional"
                      ? Teacher.regionalLanguage
                      : Teacher.language}{" "}
                  </div>
                </div>
              </div>
            ...
            <div className="xl:w-1/2 ps-3 order-1 md:order-2 flex w-full items-center justify-center md:w-[34%] lg:w-[35%]">
              <div className="xl:w-[500px] lg:w-[400px] w-[350px] h-[250px] lg:h-[300px] xl:h-[350px] rounded-lg overflow-hidden">
                <img className="w-full h-full" src={TeacherData.logo} alt="" />
              </div>
            </div>
          </div>
          <FAQ />
