...
function ViewResults() {
  ...
  const [reslutsPerPage] = useState(10);
    ...
    const fetchResults = async (id) => {
      try {
        const response = await fetch(
          `${api}/viewResults/${id}?page=${currentPage}&limit=${reslutsPerPage}`
        );
  ...
  const handleEdit = async (id, status) => {
    try {
      let resp = await fetch(`${api}/update-status`, {
        ...
        body: JSON.stringify({ status, student: "Results", id }),
      });
  ...
  const totalPages = Math.ceil(totalResults / reslutsPerPage);
                  ...
                  {results.map((result, key) => (
                    <div key={key} className="grid grid-cols-8 gap-4 w-full">
                      <div className="col-span-1 px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                        {reslutsPerPage * (currentPage - 1) + key + 1}
                      </div>
                      <div className="col-span-1 px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                        {result.result_name}
                      </div>
                      <div
                        ...
                        onClick={() => {
                          window.open(result.result_file_path, "_blank");
                        }}
                      >
                        {result.result_file}
                      </div>
