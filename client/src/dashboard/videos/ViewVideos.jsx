...
function ViewVideos() {
    ...
    const fetchResults = async (id) => {
      try {
        const response = await fetch(
          `${api}/viewVideos/${id}?page=${currentPage}&limit=${videosPerPage}`
        );
  ...
  const handleDelete = async (id) => {
    try {
      ...
      const data = await response.json();
      if (data.status) {
        setVideos(videos.filter((result) => result._id !== id));
  ...
  const handleEdit = async (id, status) => {
    try {
      let resp = await fetch(`${api}/update-status`, {
        ...
        body: JSON.stringify({ status, student: "Videos", id }),
      });
            ...
            <div className="w-[95%] border border-gray-200 m-auto overflow-x-auto ">
              <div className="w-full bg-white ">
                ...
                <div className="divide-y divide-gray-200">
                  {videos.map((result, key) => (
                    <div key={key} className="grid grid-cols-7 gap-4 w-full">
                      <div className="col-span-1 px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                        {videosPerPage * (currentPage - 1) + key + 1}
                      </div>
                      <div className="col-span-1 px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                        <Link target="_blank" to={result.video_link}>
                        {result.video_name}
                        </Link>
                      </div>
                      ...
                      <div className="col-span-2 px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        <div className="flex gap-2">
                          ...
                          <button
                            onClick={() => {
                              handleEdit(result._id, result.status);
                            }}
